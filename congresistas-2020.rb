...
congresistas_links = document.css("table.congresistas > tr").slice(1..).map do |tr|
  td = tr.css("td:nth-child(2) a")

  { identificador: td.text.parameterize, href: "http://www.congreso.gob.pe/pleno/congresistas/#{td.attribute("href").value}" }
end
...
codigos.each do |codigo|
  ...
  if congresista_info = congresistas_links.find { |link| link[:identificador] == identificador }
    request = HTTP.get(congresista_info[:href])
    document = Nokogiri::HTML(request.body.to_s)
    website = document.css("div.dpersonales > p.web > span.value > a")&.attribute("href")&.value
    bancada = document.css("div.dgenerales > p.bancada > span.value")&.text

    congresista.merge!(website: website, bancada: bancada)
  end
